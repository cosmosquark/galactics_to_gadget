...
!allocate(r_gas_sort(halogastemp))
!allocate(id_gas_sort(halogastemp))
!do i = 1, nhalogastemp
...
!do i = 1, nhalogastemp
!    rf_gas_arr_one(1:8,i) = temphaloinit(:,id_gas_sort(i))
!enddo


!! initial array is now sorted
!temphaloinit(:,:) = rf_gas_arr_one(1:8,:)
...
!allocate(dr_bin_val(nbin_rtrunc))
!allocate(dr_trunc_comp(nbin_rtrunc))

!! the r bins
!do i = 1, nbin_rtrunc
!   dr_bin_val(i) = rf_trunc_start + (i *  rf_gas_width) - (rf_gas_width / 2.0)
!   realdummy = (dr_bin_val(i) - rf_trunc) / (sqrt(2.0) * rf_trunc)
...
!       ngashalo = ngashalo + dr_bin_keep(i)
!   else
...
!
!  rf_gas_arr_one(1:8,:) = temphalogasinit(:,ivar+1:ivar+dr_bin_count(1))
!!  !! to make sure we don't break the arrays
!  rf_gas_arr_one(1:8,ivar+dr_bin_count(1)+1) = 0.0 
!
!  !! pop a value from the array
!
!  idummy = dr_bin_count(1)
!
!  do while (idummy > dr_bin_keep(i))
!     irand = int(rand(0)*idummy)+1
!     !! shift everything down one from irand.. effectively overwriting irand
!     rf_gas_arr_one(1:8,irand:dr_bin_count(1)) = rf_gas_arr_one(1:8,irand+1:dr_bin_count(1)+1)
!     idummy = idummy - 1
!  enddo

!  temphalogas(:,dr_bin_count_cum(j):dr_bin_count_cum(j)+dr_bin_keep(j)) = rf_gas_arr_one(,:dr_bin_keep(j)) 
!  deallocate(rf_gas_arr_one)
